...
contract Card{
  ...
  //  mapping(roomcount=>Room[])) playermap;
    ...
    uint index=0;
    uint firstplayer;
    event RpEvent(address fromA,address fromB,uint256 index, uint256 length);
   ...
   function getselfaddress()public payable{
       ...
       RpEvent(playerpool[index],playerpool[index+1],index,playerpool.length);
      ...
      rp.playerA[playerpool[index]]=1*10**16;
      pa=rp.playerA[playerpool[index]];
      rp.playerB[playerpool[index+1]]=1*10**16;
       roommoney.push(rp);
      index+=2;
      
    //  roommoney[index/2].roomid=rm.roomid;
    //      roommoney[index/2].playeraddr[index]=playerpool[index];
    //      roommoney[index/2].playeraddr[index+1]=playerpool[index+1];
    //   roommoney[index/2].playerA[playerpool[index]]=1*10**16;
    //   roommoney[index/2].playerB[playerpool[index+1]]=1*10**16;
  ...
  //未配对玩家池
   ...
   function getindex() public view returns(uint){
     ...
     uint public money1;
    uint public hym;
   function MoneybyRoomId(uint roomId)public {
       hym=roommoney[roomId].roomid;
        money1=this.balance;

   }

   function getRoomMoney()public returns(uint){
     return money1;
   }


   //给个房间号，赢得玩家地址 返回金额
   function PayWinMoney(address add)public {
       add.transfer(money1);
   }
}
